...
it('can add the star name and star symbol properly', async () => {
    ...
    let instance = await StarNotary.deployed();
    await instance.createStar('Awesome Star!', 23, { from: accounts[0] })
    assert.equal(await instance.name.call(), "Asish Star Token")
    assert.equal(await instance.symbol.call(), "AST")
...
it('lets 2 users exchange stars', async () => {
    ...
    let user2 = accounts[2];
    let instance = await StarNotary.deployed();
    await instance.createStar('One Big Star', tokenId1, { from: user1 })
    await instance.createStar('Another Awesome Star', tokenId2, { from: user2 })
    // 2. Call the exchangeStars functions implemented in the Smart Contract
...
it('lookUptokenIdToStarInfo test', async () => {
    ...
    let user = accounts[0];
    let starId = 6;
    await instance.createStar('Dongo Star', starId, { from: user });
    let result = await instance.lookUptokenIdToStarInfo(starId);
    assert.equal(result[0], 'Dongo Star');
